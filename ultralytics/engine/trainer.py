...
class loss_disillation:
    ...
    def __init__(self, model_t, model_s, loss_type='CWDLoss'):
        ...
        # teacher: yolo8 student: yolo8  
        # self.student_match_layer_id = [6, 12, 15, 18, 21]
        # self.teacher_match_layer_id = [6, 12, 15, 18, 21]

        # teacher: yolo11 student: yolo8  
        self.student_match_layer_id = [6, 12, 15, 18, 21]
        self.teacher_match_layer_id = [6, 13, 16, 19, 22]
        ...
        for i in self.teacher_match_layer_id:
            if isinstance(seq[i], C3k2):
                channels_t.append(seq[i].cv2.conv.out_channels)

        seq = model_s.model
        for i in self.student_match_layer_id:
        ...
        for i in self.teacher_match_layer_id:
            if isinstance(seq[i], C3k2):
                self.paired_t.append(seq[i].cv2)

        seq = model_s.model
        for i in self.student_match_layer_id:
            if isinstance(seq[i], C2f):
                self.paired_s.append(seq[i].cv2)
    ...
    def get_loss(self):
        ...
        if len(self.outputs_t) > len(self.student_match_layer_id):
            self.teacher_outputs = self.teacher_outputs[len(self.student_match_layer_id):]
